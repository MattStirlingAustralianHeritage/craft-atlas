...
export const metadata = {
  title: 'List Your Studio — Craft Atlas',
  description: "Get your studio, workshop or gallery on Australia's most comprehensive directory of makers and studios.",
}
...
const TIERS = [
  {
    ...
    features: ['Basic listing', 'Pin on the map', 'Studio type & location', '1 photo', 'Claim your listing'],
  ...
  },
  {
    ...
    price: '99',
    period: 'per year',
    description: 'Everything you need to stand out and attract visitors.',
    features: ['Everything in Free', 'Unlimited photos', 'Opening hours & contact details', 'Practice description', 'Experiences & classes', 'Commissions available', 'Events calendar', 'Seasonal highlights', 'Special offers & promotions', 'Enlarged map pin', 'Featured on homepage', 'Priority placement in search', 'Analytics dashboard', 'Featured in regional guides & maker trails'],
    cta: 'Get Started',
    href: '/vendor/login',
    highlight: true,
  },
...
const ALL_FEATURES = [
  ...
  { label: 'Searchable by craft, state & region', basic: true, standard: true },
  { label: 'Studio name & location', basic: true, standard: true },
  { label: '1 photo', basic: true, standard: false },
  { label: 'Unlimited photos', basic: false, standard: true },
  { label: 'Opening hours', basic: false, standard: true },
  { label: 'Studio features & facilities', basic: false, standard: true },
  { label: 'Practice description', basic: false, standard: true },
  { label: 'Experiences & classes', basic: false, standard: true },
  { label: 'Commissions available', basic: false, standard: true },
  ...
  { label: 'Featured in regional guides & maker trails', basic: false, standard: true },
]

function Check({ included }) {
  if (included) return <span style={{ color: 'var(--primary)', fontSize: 15 }}>✓</span>
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh' }}>
      ...
      <section style={{ padding: '90px 24px 80px', textAlign: 'center', borderBottom: '1px solid var(--border)', background: 'var(--bg-2)' }}>
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          <div style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--primary)', marginBottom: 16, fontFamily: 'var(--font-sans)' }}>For Makers</div>
          <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(32px, 5vw, 54px)', fontWeight: 400, color: 'var(--text)', lineHeight: 1.15, marginBottom: 24 }}>
            Put your studio on Australia&apos;s craft map
          </h1>
          <p style={{ fontSize: 17, color: 'var(--text-2)', lineHeight: 1.7, marginBottom: 40, fontFamily: 'var(--font-sans)' }}>
            Craft Atlas is the most comprehensive directory of makers, artists and studios in Australia — and growing. Claim your free listing or upgrade for photos, featured placement, and more.
          </p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/claim" style={{ display: 'inline-block', padding: '14px 32px', background: 'var(--primary)', color: 'var(--bg)', textDecoration: 'none', fontSize: 12, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', fontFamily: 'var(--font-sans)', borderRadius: 2 }}>Claim Your Listing</Link>
            <Link href="/vendor/login" style={{ display: 'inline-block', padding: '14px 32px', background: 'transparent', color: 'var(--text)', textDecoration: 'none', fontSize: 12, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', fontFamily: 'var(--font-sans)', borderRadius: 2, border: '1px solid var(--border)' }}>Maker Login</Link>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '72px 24px', borderBottom: '1px solid var(--border)', background: 'var(--bg-2)' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <div style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--primary)', marginBottom: 14, fontFamily: 'var(--font-sans)', textAlign: 'center' }}>Pricing</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 24 }}>
            {TIERS.map(tier => (
              <div key={tier.name} style={{ padding: '36px 32px', background: tier.highlight ? 'var(--text)' : 'var(--bg)', border: tier.highlight ? 'none' : '1px solid var(--border)', borderRadius: 3, position: 'relative' }}>
                ...
                <div style={{ fontSize: 11, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'var(--primary)', fontFamily: 'var(--font-sans)', marginBottom: 12 }}>{tier.name}</div>
                ...
                <div style={{ marginBottom: 32 }}>
                  {tier.features.map(f => (
                    <div key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 10 }}>
                      <span style={{ color: 'var(--primary)', fontSize: 14, lineHeight: 1.4, flexShrink: 0 }}>✓</span>
                ...
                <Link href={tier.href} style={{ display: 'block', padding: '13px 24px', background: tier.highlight ? 'var(--primary)' : 'transparent', color: tier.highlight ? 'var(--bg)' : 'var(--text)', textDecoration: 'none', fontSize: 11, fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', fontFamily: 'var(--font-sans)', borderRadius: 2, border: tier.highlight ? 'none' : '1px solid var(--border)', textAlign: 'center' }}>{tier.cta}</Link>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: '72px 24px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <div style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--primary)', marginBottom: 14, fontFamily: 'var(--font-sans)', textAlign: 'center' }}>What&apos;s Included</div>
          ...
          <div style={{ border: '1px solid var(--border)', borderRadius: 3, overflow: 'hidden', background: 'var(--bg)' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr repeat(2, 140px)', borderBottom: '1px solid var(--border)', background: 'var(--bg-2)' }}>
              ...
              {['Free', 'Standard'].map(t => (
                <div key={t} style={{ padding: '16px 8px', textAlign: 'center', fontSize: 11, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--primary)', fontFamily: 'var(--font-sans)' }}>{t}</div>
              ))}
            </div>
      ...
      <section style={{ padding: '72px 24px', borderBottom: '1px solid var(--border)', background: 'var(--bg-2)' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <div style={{ fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--primary)', marginBottom: 14, fontFamily: 'var(--font-sans)', textAlign: 'center' }}>How It Works</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 24 }}>
            {[
              { n: '01', title: 'Find your listing', desc: 'Search for your studio or maker profile in the directory.' },
              ...
              <div key={step.n} style={{ padding: '28px 24px', border: '1px solid var(--border)', borderRadius: 3, background: 'var(--bg)' }}>
                <div style={{ fontFamily: 'var(--font-serif)', fontSize: 28, color: 'var(--primary)', opacity: 0.6, marginBottom: 12, lineHeight: 1 }}>{step.n}</div>
      ...
      <section style={{ padding: '80px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          ...
          <Link href="/claim" style={{ display: 'inline-block', padding: '16px 40px', background: 'var(--primary)', color: 'var(--bg)', textDecoration: 'none', fontSize: 12, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', fontFamily: 'var(--font-sans)', borderRadius: 2 }}>Claim Your Free Listing</Link>
          <div style={{ marginTop: 20, fontSize: 13, color: 'var(--text-3)', fontFamily: 'var(--font-sans)' }}>
            Already have an account? <Link href="/vendor/login" style={{ color: 'var(--primary)', textDecoration: 'none' }}>Sign in</Link>
          </div>
        </div>
      </section>
