...
const LISTING_COLS = [
  ...
  'operator_highlights',
  'opening_hours', 'opening_hours_status',
].join(', ')
...
export function mapListing(listing, local = null) {
  ...
  return {
    ...
    // Google/operator/website opening hours (portal listings.opening_hours).
    // Kept under a distinct key so it does not clobber the derived
    // `opening_hours` {day:value} map the existing hours block renders.
    google_opening_hours: listing.opening_hours || null,
    opening_hours_status: listing.opening_hours_status || null,
