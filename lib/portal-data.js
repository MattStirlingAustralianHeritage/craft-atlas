 ...
 *  Never use createServerSupabase() on public render paths — it calls
 *  cookies(), which silently opts ISR pages out of caching (per-request
 *  render forever) and 500s an ISR render. */
...
export async function getPortalVenue(slug) {
  ...
  let local = null
  try {
    const sb = getLocalAnon()
    if (!sb) throw new Error('local supabase env not configured')
...
//    this listing (portal type='operator'). Read off the curated public views
//    (portal migration 189) via the anon portal key — the base trails tables
//    stay RLS-locked. Stops link to the canonical aggregator place page so they
//    resolve from this vertical too. Returns null when none / fewer than 2 stops.
export async function getPortalListingTrail(listingId) {
  const portal = getPortal()
  if (!portal || !listingId) return null

  const { data: trail } = await portal
    .from('operator_trails_public')
    .select('id, title, intro, region, stop_count')
    .eq('listing_id', listingId)
    .maybeSingle()
  if (!trail) return null

  const { data: rows } = await portal
    .from('operator_trail_stops_public')
    .select('trail_id, position, editorial_copy, listing_id, venue_name, venue_slug, vertical, sub_type, venue_lat, venue_lng, venue_image_url')
    .eq('trail_id', trail.id)
    .order('position', { ascending: true })

  const stops = (rows || [])
    .filter(s => s.venue_slug && s.venue_lat != null && s.venue_lng != null)
    .map(s => ({
      ...
      vertical: s.vertical,
      sub_type: s.sub_type,
      editorial_copy: s.editorial_copy,
      venue_lat: s.venue_lat,
      venue_lng: s.venue_lng,
      place_href: `${PORTAL_BASE}/place/${s.venue_slug}`,
    }))

  if (stops.length < 2) return null
  return { title: trail.title, intro: trail.intro, region: trail.region, stops }
}
...
// Newest listings for the homepage "living index" ticker — real names from
// the live index, newest first. Lightweight columns only.
export async function listPortalRecent(limit = 20) {
  const portal = getPortal()
  if (!portal) return []
  const { data } = await applyVertical(
    portal.from('listings').select('id, name, slug, sub_type, region, state').eq('status', 'active')
  ).order('created_at', { ascending: false }).limit(limit)
  return (data || []).map(l => ({
    id: l.id, name: l.name, slug: l.slug,
    category: l.sub_type || null, region: l.region || null, state: l.state || null,
  }))
}
